...
// ✅ Categories that should ALWAYS show in home page as category pills
...
const CategoryPills = () => {
  ...
  // ✅ FIXED: Determine main categories - ONLY show DEFAULT_CATEGORIES
  // This prevents custom categories like "platter" from appearing as category pills
  const mainCategories = allCategories.filter((cat) => {
    // ONLY show categories that are in DEFAULT_CATEGORIES list
    if (DEFAULT_CATEGORIES.includes(cat)) return true;
    
    // Hide drink subcategories (they appear under DRINKS)
    if (["coffee", "fruittea", "milktea"].includes(cat)) return false;
    
    // ✅ CRITICAL FIX: Hide ALL other categories (custom categories)
    return false;
  });
  ...
  return (
    <>
      ...
      <div className="category-pills-container" style={styles.container}>
        ...
        {/* Main Categories - ONLY DEFAULT_CATEGORIES */}
